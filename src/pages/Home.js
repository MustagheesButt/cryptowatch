...
export default function Home() {
  ...
  const renderedPosts = (posts) => posts.map(p => {
    return (
      <Link to={`/${lang}/${p.slug}`} key={`${p.id}`}>
        <div className='rounded p-5 bg-white dark:bg-slate-900 hover:bg-blue-100 hover:dark:bg-blue-900 mb-5'>
  ...
  return (
    <>
      <Head title={'Crypto, Metaverse, NFTs and Pakistan'} />
      <Layout>
        ...
        {/* TODO <section className='p-10'>
          <h3 className='text-3xl mb-5'>Trending</h3>
        </section> */}

        <section className='p-10'>
          <h3 className='text-3xl mb-5'>Latest</h3>
          {isLoading ? <Loader /> : (isError ? 'Something went wrong, please try again later.' : renderedPosts(posts))}
        </section>
      </Layout>
    </>
  )
